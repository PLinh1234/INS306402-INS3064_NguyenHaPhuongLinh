...
} catch (Exception $e) { die('Không lấy được dữ liệu sinh viên.'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            if ($existing) $errors['email'] = 'Email đã thuộc về sinh viên khác.';
            else {
                $db->update('students', ['name'=>$name,'email'=>$email], 'id=?', [$id]);
                header('Location: index.php?updated=1'); exit;
            }
        } catch (Exception $e) { $errors['general'] = 'Có lỗi khi cập nhật, vui lòng thử lại.'; }
    }
}
?>
<!DOCTYPE html>
<html lang="vi">
<head><meta charset="UTF-8"><title>Sửa sinh viên</title></head>
<body>
<h1>Sửa sinh viên</h1>
